...
#### 🍀 문제를 해결했던 부분
프로젝트를 진행하면서 맞닥뜨린 문제 중 하나는 틴플레이, 유튜브 쇼츠 같은 짧은 데이터를 불러와서 위로 슬라이드(Wheel) 을 위 아래로 움직였을 때 동영상이 재생되는 기능과 관련된 이슈였습니다.
이 문제는 사용자가 슬라이드(Wheel) 조작을 통해 최신 데이터를 불러올 때 기존 정지되어있던 영상이 다시 재생되는 현상이 발생했습니다. 
이러한 문제를 해결하기 위해 코드를 면밀히 검토했고, Fetch 통신 후 변수 재정의 과정에서 오류가 발생한다는 것을 발견했습니다.

코드의 모듈화 부재와 Fetch 통신 후 변수 재정의, 반복된 조건식 처리가 문제의 원인이였습니다.
따라서 해당 문제를 해결하기 위해 Fetch 통신 관련 코드를 모듈화하고, 코드 중복을 최소화하기 위해 함수를 선언하여 사용했습니다.
또한 기존에 인덱스 번호를 이용한 방식에서 closest 메서드를 활용한 동적 요소 관리로 전환했습니다.

그리고 틴플레이 재생 기능의 개선도 필요했습니다.
사용자가 선택한 번호에 따라 해당 틴플레이를 날짜별로 정렬하여, wheel 조작에 따라 영상을 동적으로 로드하는 과정에서 날짜 순으로 정렬하여 순차 번호를 부여하는 방법을 도입했습니다.
실제 데이터 처리 시, DB에서 순차적인 PK를 기반으로 하지 않고, 사용자 선택에 따라 날짜별로 영상을 정렬하고 RowNumber() 메소드를 사용해 신규 리스트에 번호를 부여한 뒤, 해당 데이터를 wheel 위치에 따라 가져오는 로직을 적용하였습니다.

이 과정을 통해 개발 과정에서의 문제 해결 능력을 한층 강화할 수 있었고, 특히 문제를 식별하고 체계적으로 접근하여 해결한 경험을 가지고 있습니다.
더불어, 프로젝트 개발에 있어서 코드의 모듈화와 최적화의 중요성을 다시 한번 깨닫게 되었습니다

#### 🍀 팀원들간의 소통&책임감의 중요성
프로젝트를 진행하며 팀원 간의 원활한 소통과 각자의 책임감이 프로젝트의 성공에 얼마나 중요한지 직접 체험했습니다. 
프로젝트 초기부터 서로 의견을 적극적으로 나누고, 초기 작업 분배를 진행하면서 팀원들과 균등하게 분배를 하며 어떠한 부분을 진행할 수 있을 지 의견 공유를 했습니다
또한, 내가 담당한 파트에 대해서는 팀원들과 지속적으로 소통하며, 필요할 때 언제든지 내 작업 과정 및 진행도에 대하여 설명할 수 있도록 준비했습니다.

처음에는 이런 과정이 작업 속도를 늦출 수 있다고 생각했지만, 결국 어느정도 작업이 안정화 되기 시작하고 모든 팀원이 자신의 업무의 흐름을 이해할 수 있을 만큼 발전했을 때, 프로젝트의 질이 크게 향상되었습니다.
유사 작업이나 응용 작업을 하는 팀원들에게 내 경험을 공유하며, 프로젝트의 흐름을 더 원활하게 만들 수 있었습니다.

나이가 가장 많은 팀원으로서, 저도 모르게 팀 내에서 좀 더 책임감 있는 자세를 가지고 업무를 진행했습니다.
팀원들이 겪는 어려움을 살피고, 내 경험을 바탕으로 지원하는 것을 중요하게 여겼습니다. 
이러한 태도는 서로를 더 신뢰하게 하고 자유롭게 의견을 나누는 분위기를 조성하는데 크게 기여했습니다.

프로젝트가 진행됨에 따라, 우리 팀의 소통은 네트워크처럼 점점 더 유기적으로 발전해 갔습니다. 
이런 유기적인 연결의식 덕분에 프로젝트를 성공적으로 마무리하고 서버 배포까지 무사히 완료될 수 있었습니다.

이 모든 경험을 통해, 각자 맡은 바 역할에 책임감을 가지고, 팀 내에서 서로에게 소통을 하는 것이 중요한 부분임을 다시 한번 깊이 깨달았습니다.
...
### 🖼️사이트 이동 <a href="http://teenplay.store/"><img src="https://img.shields.io/badge/Teenplay-EE0000?style=plastic&logo=paypal&logoColor=white"/></a>
...
-->
    <tr>
        <td><img width="713" alt="스크린샷 2024-04-12 오전 12 09 39" src="https://github.com/kimgusan/teenplay_server/assets/156397911/9b0f19ea-b693-4bca-863e-9d613cbd97b7"></td>
        <td><img width="680" alt="스크린샷 2024-04-12 오전 12 10 08" src="https://github.com/kimgusan/teenplay_server/assets/156397911/952ac840-ac27-47c6-9a1e-0a3c81a422dc"></td>
    </tr>
    <tr>
        <td><img width="729" alt="스크린샷 2024-04-12 오전 12 10 16" src="https://github.com/kimgusan/teenplay_server/assets/156397911/137954b2-3d78-4b11-aaa4-57cc9114d95b"></td>
        <td><img width="699" alt="스크린샷 2024-04-12 오전 12 10 29" src="https://github.com/kimgusan/teenplay_server/assets/156397911/d6bc9a20-c8a2-4be7-bc4e-dbf48d8c578d"></td>
    </tr>
</table>
<br>
